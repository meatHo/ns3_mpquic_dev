...
def main():
    ...
    # ========================================================== #
    # <<< 여기에 저장된 모델을 불러오는 코드를 추가합니다 >>>
    model_path = "./real_final2/mine/fine1/dqn_ns3_final_fine1.pth"  # 불러올 모델의 경로
    try:
        agent.q.load_state_dict(torch.load(model_path, map_location=device))
        # 타겟 네트워크도 똑같이 맞춰주는 것이 안정적입니다.
        agent.q_target.load_state_dict(agent.q.state_dict())
        print(f"✅ 저장된 모델 '{model_path}'을(를) 성공적으로 불러왔습니다. 추가 학습을 시작합니다.")
    except FileNotFoundError:
        print(f"⚠️ 모델 파일을 찾을 수 없습니다: '{model_path}'. 처음부터 학습을 시작합니다.")
    # ========================================================== #
    ...
    for ep in range(MAX_EPISODES):
        ...
        # 체크포인트 저장
        if ep % 10 == 0:
            ckpt_name = f"./real_final2/mine/fine2/checkpoint_ep{ep}.pth"
    ...
    # 최종 모델 저장
    torch.save(agent.q.state_dict(), "./real_final2/mine/fine2/dqn_ns3_final_fine1.pth")
