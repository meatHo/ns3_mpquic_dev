...
lastStat()
{
    ...
    std::ofstream outFile("/home/kiho/ns-3-quic_copy/contrib/opengym/package.json");
...
// static Ptr<OpenGymInterface> openGym = CreateObject<OpenGymInterface> (5557);
...
TotalParameter(Ptr<Node> node, KStats stats)
{
    ...
    {
        prr = static_cast<double>(recv) / 90.0;
    }
...
main(void)
{
    ...
    nrHelper->SetUeBwpManagerAlgorithmAttribute("GBR_LIVE_UL_71", UintegerValue(bwpIdForGbrMcptt));
    ...
    for (uint16_t i = 0; i < ueNodeContainer.GetN(); ++i)
    {
        ...
        Simulator::Schedule(Seconds(9.0), &UdpKohClient::clearCount, clientApp); // 학습시작전
                                                                                 // 초기화
