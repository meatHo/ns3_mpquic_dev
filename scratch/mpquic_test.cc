 ...
 *   \              /
 *     rsu --------
 *     ue-> serve은 멀티캐스트 반대는 unicast
 */

#include "../src/koh/kohTag.h"
#include "../src/quic/model/quic-socket-factory.h"
...
lastStat()
{
    ...
    {
        NS_LOG_UNCOND(
            ">>>> FAILED to open sim_results.json for writing! Check permissions or path. <<<<");
    }
    // ==========================================================
    outFile << "{" << std::endl;
    ...
    std::cout << "  \"Total PRR\": " << receptionRatio * 100 << std::endl;
}
...
static uint32_t before = 0;
static Time minDwell = MilliSeconds(1); // 전환 최소 유지시간 (옵션)
static Time lastSwitchTime = Seconds(0);

void
ApplyInterfaceSwitch(uint32_t a, uint16_t ueId)
{
    ...
    // Simulator::Schedule(slotDur, &ResumeTraffic, ueId);
}
...
ExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    uint32_t a = discrete->GetValue();
    uint16_t ueId = g_targetUeId;
    NS_LOG_UNCOND("ExecuteActions at " << Simulator::Now().GetSeconds() << " action=" << a);

    if (a == before)
        return true;

    // 최소 유지시간 가드(토글 과민 방지)
    if (Simulator::Now() - lastSwitchTime < minDwell)
        return true;

    // 다음 슬롯 경계까지 지연
    double mu = 1.0;                               // gNB numerology
    double slotDurSec = 0.001 / std::pow(2.0, mu); // 0.5ms
    double now = Simulator::Now().GetSeconds();
    double rem = std::fmod(now, slotDurSec);
    double delay = (rem == 0.0) ? 0.0 : (slotDurSec - rem);

    Simulator::Schedule(Seconds(delay), &ApplyInterfaceSwitch, a, ueId);
    before = a;
    return true;
}
...
TotalParameter(Ptr<Node> node, KStats stats)
{
    ...
    if (before == 0)
    {
        prr = static_cast<double>(recv) / 900.0;
    }
    else
    {
        prr = static_cast<double>(recv) / 900.0;
...
// {
//     // std::cout << "📶 Sl [Meas] RNTI=" << RNTI << " L2ID=" << L2ID << " RSRP=" << RSRP << "
...
//     NS_LOG_UNCOND("UE Velocity: x=" << vel1.x << ", y=" << vel1.y << ", z=" << vel1.z << "
//     (m/s)"); NS_LOG_UNCOND("Vehicle 2"); NS_LOG_UNCOND("UE Position: x=" << pos2.x << ", y=" <<
//     pos2.y << ", z=" << pos2.z); NS_LOG_UNCOND("UE Velocity: x=" << vel2.x << ", y=" << vel2.y <<
//     ", z=" << vel2.z << " (m/s)"); Simulator::Schedule(Seconds(1.0), &PrintUeInfo, ueNode1,
//     ueNode2);
// }
...
void
Ipv4PacketTraceAtPgw(Ptr<const Packet> packet, Ptr<Ipv4> Ipv4, uint32_t interfaceIndex)
{
    Ipv4Header Ipv4Header;
    packet->PeekHeader(Ipv4Header);
...
void
Ipv4PacketTraceAtRouter(Ptr<const Packet> packet, Ptr<Ipv4> Ipv4, uint32_t interfaceIndex)
{
    Ipv4Header Ipv4Header;
    packet->PeekHeader(Ipv4Header);
...
void
Ipv4PacketTraceAtServer(Ptr<const Packet> packet, Ptr<Ipv4> Ipv4, uint32_t interfaceIndex)
{
    Ipv4Header Ipv4Header;
    packet->PeekHeader(Ipv4Header);
...
void
Ipv4PacketTraceAtUe(Ptr<const Packet> packet, Ptr<Ipv4> Ipv4, uint32_t interfaceIndex)
{
    Ipv4Header Ipv4Header;
    packet->PeekHeader(Ipv4Header);
...
main(void)
{
    ...
    gnbNodeContainer.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(905, 4722, 80.0));
    mobility.Install(rsuNodeContainer);
    rsuNodeContainer.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(290.0, 4197.0, 70.0));
    ...
    nrHelper->GetGnbPhy(gnbNetDev.Get(0), 0)->SetNoiseFigure(12.0);
    ...
    nrHelper->SetUeMacAttribute("T2", UintegerValue(11));
    ...
    Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize",
                       UintegerValue(100000)); // koh 이거 관련있음
    ...
    ptrFactory->SetSlSubchannelSize(15);
    ...
    tddUlDlConfigCommon.tddPattern = "UL|UL|UL|UL|UL|UL|UL|UL|UL|UL"; // koh tddpattern
    ...
    psschParams.slMaxTxTransNumPssch = 1; // harq 숫자 기본값5인데 낮춤.
    ...
    // // UE와 RSU 양쪽 모두에 BIDIRECTIONAL로 설정
    // Ptr<LteSlTft> tft_rsrp =
    //     Create<LteSlTft>(LteSlTft::Direction::BIDIRECTIONAL, rsrpAddress, slInfo);
    ...
    quic.InstallQuic(rsuNodeContainer);
    ...
    slInfo1.m_rri = MilliSeconds(10);
    slInfo1.m_pdb = delayBudget;
    slInfo1.m_dynamic = false;
    slInfo1.m_harqEnabled = true;

    tft1 = Create<LteSlTft>(LteSlTft::Direction::BIDIRECTIONAL, ue1SlIp, slInfo1);
    // nrSlHelper->ActivateNrSlBearer(Seconds(0.0), rsuRsrpNetDev, tft1);
    nrSlHelper->ActivateNrSlBearer(Seconds(1.0), rsuNetDev, tft1);
    ...
    Simulator::Schedule(Seconds(3.0), &QuicKohServer::clearCount, serverApp, 0); // 학습시작전 초기화
    ...
    for (uint16_t i = 0; i < ueNodeContainer.GetN(); ++i)
    {
        ...
        // Simulator::Schedule(Seconds(5.0), &UdpKohClient::clearCount, clientApp); // 학습시작전
        // 초기화
        // Simulator::Schedule(Seconds(10.0), &TotalParameter, clientApp, serverApp,
    ...
    rsuApp->SetAttribute("Protocol", TypeIdValue(QuicSocketFactory::GetTypeId()));
    ...
    for (uint16_t ueIndex = 0; ueIndex < ueUuNetDev.GetN(); ++ueIndex)
    {
        ...
        uePhy->TraceConnectWithoutContext("ReportRsrp", MakeCallback(&UeMeasCallback));
    }
    ...
    for (uint64_t ueIndex = 0; ueIndex < ueSlNetDev.GetN(); ++ueIndex)
    {
        ...
        uePhy->TraceConnectWithoutContext("ReportSlRsrp", MakeCallback(&UeSlMeasCallback));
    }
  ...
  public:
    ...
    inline void PrintRoutingTable(Ptr<Node>& n)
    {
        ...
        for (uint32_t i = 0; i < nbRoutes; i++)
        {
            ...
            std::cout << route.GetDest() << "\t" << route.GetDestNetworkMask() << "\t"
                      << route.GetGateway() << "\t" << route.GetInterface() << std::endl;
        }
        std::cout << "------------------------------------" << std::endl;
    }
};
