...
main(int argc, char* argv[])
{
    ...
    uint32_t udpPacketSizeBe = 1000; // koh 1500 최대 slbitmap, tddpattern수정시 2000이 최대
    double dataRateBe = 9000;        // 16 kilobits per second //koh
    ...
    uint16_t numerologyBwpSl = 0;           // koh
    double centralFrequencyBandSl = 5.89e9; // band n47  TDD //Here brand is analogous to channel
    ...
    Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize",
                       UintegerValue(999999999)); // koh 이거 관련있음
    ...
    nrHelper->SetUeMacAttribute("EnableSensing", BooleanValue(false)); // koh 이거 별로 상관없는데?
    nrHelper->SetUeMacAttribute("T1", UintegerValue(2));
    nrHelper->SetUeMacAttribute("T2", UintegerValue(11)); // koh
    ...
    nrSlHelper->SetUeSlSchedulerAttribute("Mcs", UintegerValue(28)); // koh
    ...
    std::list<uint16_t> resourceReservePeriodList = {0, 10}; // in ms //koh
    ...
    slInfo.m_rri = MilliSeconds(10); // koh
