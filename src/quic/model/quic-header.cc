...
QuicHeader::TypeToString() const
{
    static const char* longTypeNames[6] =
        {"Version Negotiation", "Initial", "Retry", "Handshake", "0-RTT Protected", "None"};
...
QuicHeader::CalculateHeaderLength() const
{
    ...
    {
        len = 8 + 64 * HasConnectionId() + GetPacketNumLen() + 8;
    }
    return len / 8;
}
...
{
    return (lhs.m_form == rhs.m_form && lhs.m_c == rhs.m_c && lhs.m_k == rhs.m_k &&
            lhs.m_type == rhs.m_type && lhs.m_connectionId == rhs.m_connectionId &&
            lhs.m_packetNumber == rhs.m_packetNumber && lhs.m_version == rhs.m_version &&
