...
QuicUdpBinding::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::QuicUdpBinding")
                            ...
                            .AddAttribute("QuicSocketBase",
                                          "The QuicSocketBase pointer.",
                                          PointerValue(),
    ...
    static TypeId tid =
        TypeId("ns3::QuicL4Protocol")
            ...
            .AddAttribute("RttEstimatorType",
                          ...
                          MakeTypeIdChecker())
            .AddAttribute("0RTT-Handshake",
                          "0RTT-Handshake start",
                          BooleanValue(false),
            ...
            .AddAttribute("SocketList",
                          "The list of UDP and QUIC sockets associated to this protocol.",
                          ObjectVectorValue(),
                          MakeObjectVectorAccessor(&QuicL4Protocol::m_quicUdpBindingList),
                          MakeObjectVectorChecker<QuicUdpBinding>())
        /*.AddAttribute ("AuthAddresses", "The list of Authenticated addresses associated to this
           protocol.", ObjectVectorValue (), MakeObjectVectorAccessor
           (&QuicL4Protocol::m_authAddresses), MakeObjectVectorChecker<Address> ())*/
        ;
    return tid;
}
...
QuicL4Protocol::SetListener(Ptr<QuicSocketBase> sock)
{
    NS_LOG_FUNCTION(this);
    // todo: 여기 수정 해야함.
    //  NS_LOG_UNCOND("QuicL4SetListenner");
...
QuicL4Protocol::ForwardUp(Ptr<Socket> sock)
{
    ...
    while ((packet = sock->RecvFrom(from)))
    {
        ...
        /*else if (m_sockets.size () <= 2) // Rivedere
          {
            ...
            else if (m_sockets.size () == 2 && m_sockets[1]->GetSocketState () !=
          QuicSocket::LISTENING)
              ...
              {
                NS_FATAL_ERROR ("The Connection ID can only be omitted by means of
          m_omit_connection_id transport parameter" " if source and destination IP address and port
          are sufficient to identify a connection");
              }

          }*/
        else
        {
            NS_FATAL_ERROR("The Connection ID can only be omitted by means of m_omit_connection_id "
                           "transport parameter"
                           " if source and destination IP address and port are sufficient to "
        ...
        {
            NS_LOG_UNCOND(this << " Cloning listening socket "
                               << m_quicUdpBindingList.front()->m_quicSocket);
            ...
            NS_LOG_UNCOND(InetSocketAddress::ConvertFrom(from).GetIpv4()
                          << "  retval : " << retval);
            socket->SetupCallback();
        }
        else if (header.IsHandshake() and m_isServer and socket)
        {
            NS_LOG_UNCOND("CONNECTION AUTHENTICATED - Server authenticated Client "
                          << InetSocketAddress::ConvertFrom(from).GetIpv4() << " port "
                          << InetSocketAddress::ConvertFrom(from).GetPort() << "");
            m_authAddresses.push_back(InetSocketAddress::ConvertFrom(from)
                                          .GetIpv4()); // add to the list of authenticated sockets
        }
        else if (header.IsHandshake() and !m_isServer and socket)
        {
            NS_LOG_UNCOND("CONNECTION AUTHENTICATED - Client authenticated Server "
                          << InetSocketAddress::ConvertFrom(from).GetIpv4() << " port "
                          << InetSocketAddress::ConvertFrom(from).GetPort() << "");
            m_authAddresses.push_back(InetSocketAddress::ConvertFrom(from)
                                          .GetIpv4()); // add to the list of authenticated sockets
        ...
        else if (header.IsORTT() and m_isServer)
        {
            ...
            // check if a 0-RTT is allowed with this endpoint - or if the attribute
            // m_0RTTHandshakeStart has been forced to be true
            if (result == m_authAddresses.end() && m_0RTTHandshakeStart)
            {
                m_authAddresses.push_back(
                    InetSocketAddress::ConvertFrom(from)
                        .GetIpv4()); // add to the list of authenticated sockets
            }
            else if (result == m_authAddresses.end() && !m_0RTTHandshakeStart)
            {
                NS_LOG_WARN(this << " CONNECTION ABORTED: 0RTT Packet from unauthenticated address "
                                 << InetSocketAddress::ConvertFrom(from).GetIpv4() << " port "
                                 << InetSocketAddress::ConvertFrom(from).GetPort());
                continue;
            }
        ...
        else if (header.IsShort())
        {
            ...
            {
                NS_LOG_WARN(
                    this << " CONNECTION ABORTED: Short Packet from unauthenticated address "
                         << InetSocketAddress::ConvertFrom(from).GetIpv4() << " port "
                         << InetSocketAddress::ConvertFrom(from).GetPort());
                continue;
            }
...
void
QuicL4Protocol::ReceiveIcmp(Ipv4Address icmpSource,
                            uint8_t icmpTtl,
                            uint8_t icmpType,
                            uint8_t icmpCode,
                            uint32_t icmpInfo,
...
void
QuicL4Protocol::ReceiveIcmp(Ipv6Address icmpSource,
                            uint8_t icmpTtl,
                            uint8_t icmpType,
                            uint8_t icmpCode,
                            uint32_t icmpInfo,
...
void
QuicL4Protocol::SendPacket(Ptr<QuicSocketBase> socket,
                           Ptr<Packet> pkt,
